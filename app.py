...
# --- 1. إعدادات الصفحة ---
st.set_page_config(page_title="Koralytics AI | Fixed", page_icon="💎", layout="wide")
    ...
    try:
        ...
        for m in r:
            ...
            if h2h:
                ...
                # قيم الفوز (H2H)
                price_1 = h2h['outcomes'][0]['price']
                price_2 = h2h['outcomes'][1]['price']
                price_x = 1.0
                if len(h2h['outcomes']) > 2:
                    price_x = h2h['outcomes'][2]['price']
...
try:
    sports_data = requests.get(f'https://api.the-odds-api.com/v4/sports/?apiKey={API_KEY}', timeout=8).json()
...
if not df.empty:
    ...
    if conf > 80:
        advice, color, bg = "🚀 فرصة ذهبية", "#16a34a", "#f0fdf4"
    elif conf > 65:
        advice, color, bg = "⚖️ استثمار متوازن", "#2563eb", "#eff6ff"
    else:
        advice, color, bg = "⚠️ مخاطرة عالية", "#dc2626", "#fef2f2"
